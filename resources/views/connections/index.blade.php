    ...
    <div class="space-y-8">
        ...
        <div class="flex gap-8 border-b border-[#e5e7eb]">
            ...
            <a class="{{ $tabClass('pending') }}" href="{{ route('connections.index', ['tab' => 'pending']) }}">
                Pending <span class="ml-2 text-[10px] px-2 py-0.5 rounded-full bg-[#f0f2f4] text-[#617589] font-black">{{ $pendingCount }}</span>
            </a>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                @forelse($friends as $u)
                    ...
                    <div class="bg-white border border-[#e5e7eb] rounded-2xl p-5 text-center hover:shadow-soft transition">
                        <div class="w-20 h-20 rounded-full mx-auto bg-cover bg-center ring-4 ring-[#f6f7f8]" style="background-image:url('{{ $avatar }}')"></div>
                ...
                @forelse($pendingReceived as $req)
                    ...
                    <div class="p-4 flex flex-wrap items-center justify-between gap-4 border-b border-[#e5e7eb] last:border-0 hover:bg-[#f8f9fb] transition">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 rounded-full bg-cover bg-center" style="background-image:url('{{ $avatar }}')"></div>
                            <div>
                                <p class="text-sm font-extrabold text-[#111418]">{{ $u->name }}</p>
                                <p class="text-xs text-[#617589]">{{ $title }}</p>
                                <p class="text-[10px] text-primary font-bold mt-1">Sent {{ $req->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
