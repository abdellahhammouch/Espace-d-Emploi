    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white dark:bg-[#1a2633] rounded-xl border border-[#e5e7eb] dark:border-[#2d3748] p-5 flex flex-col items-center text-center shadow-sm">
            ...
            <a href="{{ route('users.show', $u) }}"
                class="flex items-center justify-center gap-1 text-lg font-bold text-[#111418] dark:text-white hover:underline">

                {{ $u->name }}

                @if($u->isCurrentlyVerified())
                <span class="material-symbols-outlined text-[#137fec] text-[20px]">
                    verified
                </span>
                @endif
            </a>
